...
# Connect to database and get Qiwi account details
conn = sqlite3.connect("data.db")
cursor = conn.cursor()
qiwi_info = cursor.execute("SELECT num, token FROM qiwi").fetchone()
qiwi_number, qiwi_token = qiwi_info[0], qiwi_info[1]
QIWI_ACCOUNT = str(qiwi_number)

# Make request to Qiwi API to get recent payments
s = requests.Session()
s.headers['authorization'] = f"Bearer {qiwi_token}"
parameters = {'rows': '50'}
url = f"https://edge.qiwi.com/payment-history/v1/persons/{QIWI_ACCOUNT}/payments"
response = s.get(url, params=parameters)
if response.status_code != 200:
    print(f"Failed to get payments from Qiwi API. Status code: {response.status_code}")
    exit()
payments = json.loads(response.text)['data']

# Process payments that match the user's comment
for payment in payments:
    comment = payment['comment']
    cursor.execute(f"SELECT * FROM oplata WHERE comment = ?", (comment,))
    result = cursor.fetchone()
    if result is not None:
        user_id, _ = result
        skolko = payment['sum']['amount']
        cursor.execute(f"DELETE FROM oplata WHERE comment = ?", (comment,))
        cursor.execute(f"SELECT balance FROM users WHERE id = ?", (user_id,))
        balancenow = cursor.fetchone()[0]
        cursor.execute(f"UPDATE users SET balance = ? WHERE id = ?", (balancenow + skolko, user_id))
        conn.commit()

        # Get the worker's information and notify them about the payment
        cursor.execute(f"SELECT boss FROM users WHERE id = ?", (user_id,))
        wk = cursor.fetchone()[0]
        cursor.execute(f"SELECT username, name FROM users WHERE id = ?", (wk,))
        worker = cursor.fetchone()
        worker_username, worker_name = worker[0], worker[1]
        cursor.execute(f"SELECT name FROM users WHERE id = ?", (user_id,))
        mamont = cursor.fetchone()[0]
        message = f"Received payment of {skolko} from {mamont} (user id: {user_id})"
        s.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
               data={"chat_id": worker_username, "text": message})
